...
# Repo.clone_from("https://github.com/PhonePe/pulse.git", "F:/GUVI/Projetcs/Phonepe_Pulse")
...
# Creating menu option in the sidebar
with st.sidebar:
    selected = option_menu("Menu", ["Home", "Top Charts", "Explore Data", "About"],
                           icons=["house", "graph-up-arrow", "bar-chart", "exclamation-circle"],
                           menu_icon="menu-button",
                           default_index=0,
                           styles={"nav-link": {"font-size": "2 0px", "text-align": "left", "--hover-color": "#600E8F"},
                                   "nav-link-selected": {"background-color": "#600E8F"}})

# Menu Option 1 - Home

if selected == "Home":
    #Img1 = Image.open("F:/GUVI/Projects/Phonepe_Pulse/phonepe-2.png")
    #st.image(Img1)
    st.markdown("# :violet[Data Visualization and Exploration]")
    st.markdown("## :violet[A User_Friendly Tool Using Streamlit and Plotly]")
    ...
    with col1:
        ...
        st.markdown(
            "### :violet[Technologies :] Github Cloning, Python, MySql, Pandas, Streamlit, Mysql-connector-python and Plotly.")
        st.markdown(
...
if selected == "Top Charts":
    ...
    if Type == "Transactions":
        ...
        with col1:
            ...
            mycursor.execute(
                f"select state, sum(Transaction_count) as Total_Transaction_Count, sum(Transaction_amount) as Total from agg_trans where year = {Year} and Quarter = {Quarter} group by state order by Total desc limit 10")
        ...
        with col3:
            ...
            mycursor.execute(
                f"select pincode, sum(Transaction_count) as Total_Transaction_Count, sum(Transaction_amount) as Total from top_trans where year = {Year} and Quarter = {Quarter} group by pincode order by Total desc limit 10")
            df = pd.DataFrame(mycursor.fetchall(), columns=["Pincode", "Transaction_Count", "Total_Amount"])
            fig = px.pie(df, values='Total_Amount',
                         names='Pincode',
                         title='Top 10',
                         color_discrete_sequence=px.colors.sequential.Agsunset,
    ...
    if Type == "Users":
        ...
        if Year == 2022 and Quarter in [2, 3, 4]:
            ...
            with col2:
                st.markdown("### :violet[District]")
                mycursor.execute((
                                     f"select district, sum(RegisteredUser) as Total_Users, avg(AppOpens) as Total_Appopens from map_user where year = {Year} and quarter = {Quarter} group by district order by Total_Users desc limit 10"))
...
if selected == "Explore Data":
    ...
    # Explore Data - Transacrions
    if Type == "Transactions":
        # Overall State Data - Transactions Amount
        with col1:
            st.markdown("## :violet[Overall State Data - Transactoins Amount]")
            mycursor.execute(
            ...
            df2 = pd.read_csv("C:/Users/Local User/PycharmProjects/statenames.csv")
            df1.State = df2
        ...
        with col2:
            st.markdown("## :violet[Overall State Data - Transactoins Count]")
            mycursor.execute(
                f"select state, sum(count) as Total_Transactions, sum(amount) as Total_Amount from map_trans where year = {Year} and quarter = {Quarter} group by state order by state")
            df1 = pd.DataFrame(mycursor.fetchall(), columns=["State", "Total_Transactions", "Total_Amount"])
            df2 = pd.read_csv("C:/Users/Local User/PycharmProjects/statenames.csv")
        ...
        # BAR CHART TRANSACTIONS - DISTRICT WISE
        st.markdown("#")
    ...
    # EXPLORE DATA - USERS
    if Type == "Users":
        # OVERALL STATE DATA - TOTAL APP OPENS
        st.markdown("## :violet[Overall State Data - User App Openind Frequency]")
        mycursor.execute(
        ...
        df2 = pd.read_csv('C:/Users/Local User/PycharmProjects/statenames.csv')
        df2.Total_Appopens = df1.Total_Appopens.astype(float)
        df1.State = df2
        ...
        # BAR CHART TOTAL USERS - DISTRICT WISE
        st.markdown("## :violet[Select any State]")
        selected_state = st.selectbox("",
                                      ('andaman-&-nicobar-islands', 'andhra-pradesh', 'arunachal-pradesh', 'assam',
                                       'bihar',
...
if selected == "About":
    ...
    with col1:
        ...
        st.write(
            "##### PhonePe is an Indian digital payments and financial technology company known for its mobile wallet and payment services. It offers a range of services, including peer-to-peer payments, bill payments, recharges, and more.")
